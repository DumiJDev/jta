...
import dev.jta.runtime.csrf.NoopCsrfTokenStore;
...
public final class JtaTestHarness {
    ...
    private CsrfTokenStore csrfTokenStore = new NoopCsrfTokenStore("X-JTA-CSRF-Token");
    private final JtaSession session = new InMemorySessionStore(Duration.ofHours(1)).getOrCreate("jta-test-harness");
    ...
    /**
     * Troca o {@link CsrfTokenStore}. O default e um
     * {@link NoopCsrfTokenStore}: um teste nao deveria ter de forjar
     * cookie + header validos so para exercitar uma acao, entao a
     * verificacao de CSRF sai do caminho por padrao. Passe um
     * {@code HmacCsrfTokenStore} aqui quando o objeto do teste for
     * justamente o comportamento de CSRF.
     */
    public JtaTestHarness withCsrfTokenStore(CsrfTokenStore csrfTokenStore) {
        this.csrfTokenStore = csrfTokenStore;
        return this;
    }

    /**
     * A {@link JtaSession} deste harness - em memoria e viva por toda a
     * duracao do harness, entao estado de sessao e mensagens flash
     * gravadas por uma acao continuam visiveis para a chamada seguinte,
     * como aconteceria numa sessao real do browser.
     */
    ...
    public JtaPageDispatcher pageDispatcher() {
        return new JtaPageDispatcher(registry, new ComponentInvoker(componentFactory), templateEngine, config, csrfTokenStore);
    }
    ...
    public JtaActionDispatcher actionDispatcher() {
        return new JtaActionDispatcher(registry, new ComponentInvoker(componentFactory), templateEngine, validator, csrfTokenStore);
    }
    ...
    public PageResult renderPage(Class<?> componentType, Map<String, String[]> queryParams,
        ...
        return pageDispatcher().dispatch(metadata(componentType), queryParams, pathVariables, user, session, null);
    }
    ...
    public ActionResult invokeAction(Class<?> componentType, String actionName, Map<String, String[]> params,
        ...
        return actionDispatcher().dispatch(selector, actionName, params, user, session, new CsrfRequest(null, null));
    }
}
