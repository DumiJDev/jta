...
public enum JtaFeature {
    ...
    /** Protecao CSRF nativa (double-submit HMAC). Ainda nao presente neste branch - ver jta-runtime/csrf noutra fase/branch. */
    CSRF("Protecao CSRF nativa (double-submit HMAC)"),

    /** Sessao agnostica de framework (JtaSession/SessionStore). Ainda nao presente neste branch. */
    SESSION("Sessao agnostica de framework"),

    /** Upload de arquivo (multipart/form-data). Nao implementado em nenhum adaptador ainda. */
    FILE_UPLOAD("Upload de arquivo (multipart/form-data)"),

    /** Composicao de componentes com passagem de argumentos ({@code @Use} + {@code @Input}). Ainda nao presente neste branch. */
